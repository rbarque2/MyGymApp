...
- **Historial y ajustes en el sistema híbrido (Fase 5)**: el historial se reescribe en Asfalto (header HISTORIAL display gigante, sesiones como filas duras con el día grande en naranja + mes mono a la izquierda, nombre en condensada, hora/duración/volumen en mono, desplegable de series mantenido con la lista en mono). Los headers de sección de Ajustes pasan al estilo del sistema (barra naranja + label mono).
- **Login, perfil y detalle de rutina en el sistema híbrido (Fase 4 — completa la migración)**: el login pasa a ser un póster de bienvenida (foto de gimnasio full-bleed, palabra de marca rotativa gigante + "EN MOVIMIENTO.", tagline y CTA blanco duro). El perfil se reescribe en Asfalto (nombre en condensada, badge de nivel, stats como números gigantes con retícula de 1px, logros en grid de bloques duros). El detalle de rutina estrena header póster con foto de portada (SliverAppBar colapsable) y lista de ejercicios dura con números naranjas grandes y metadatos mono. Con esto, las pantallas principales (Home, Workout, Stats, Cierre, Login, Perfil, Detalle) comparten el sistema.
- **Stats y cierre de sesión en Asfalto (Fase 3 del sistema híbrido)**: la pantalla de progreso se reescribe con números gigantes en Barlow Condensed (racha como protagonista con borde naranja, bloques de mes con retícula de 1px, barras del gráfico cuadradas con labels mono, filas de total/historial duras con fecha mono). La pantalla de fin de entreno pasa a ser un póster compartible (cabina oscura, duración gigante como protagonista, cifras en retícula, botón COMPARTIR que copia el resumen al portapapeles). Limpieza: eliminado `_stepButton` muerto en workout_screen. El preview (`lib/main_preview.dart`) ahora trae un launcher para revisar Home/Stats/Cierre sin login.
